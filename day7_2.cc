...
bool is_possible(long long res, const vector<long long>& vals)
{
    ...
    vector<long long> cat = {stoll(to_string(num1) + to_string(num2))};
    add.insert(add.end(), slice.begin(), slice.end());
    mul.insert(mul.end(), slice.begin(), slice.end());
    cat.insert(cat.end(), slice.begin(), slice.end());
    return is_possible(res, add) || is_possible(res, mul) || is_possible(res, cat);
}

int main()
{
    const char* filename = "inputs/day7.txt";
