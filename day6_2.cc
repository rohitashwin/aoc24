...
map_node orig_map[map_width][map_height];
...
bool solve()
{
    ...
    while (true)
    {
        ...
        if(is_cycle(row, col, current_direction))
        {
            return true;
        }
...
void copy_map()
{
    for(int row = 0; row < map_height; ++row)
    {
        for(int col = 0; col < map_width; ++col)
        {
            pat_map[row][col].direction = orig_map[row][col].direction;
            pat_map[row][col].obstacle = orig_map[row][col].obstacle;
            pat_map[row][col].visited = orig_map[row][col].visited;
        }
    }
}
...
int main()
{
    ...
    for(int row = 0; row < map_height; ++row)
    {
        for(int col = 0; col < map_width; ++col)
        {
            ...
            if(orig_map[row][col].obstacle)
            {
                continue;
            }
            copy_map();
            current_direction = start_direction;
            pat_map[row][col].obstacle = true;
            if(solve())
            {
                sum++;
            }
